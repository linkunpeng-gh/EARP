...
EXPECTED_TABLES = 43  # tenants+baseline+0005/6/8/9/14+0018(tbox_changes)+0019(eval 4 表)
...
def test_upgrade_idempotent_downgrade_and_seed(fresh_db_url: str) -> None:
    ...
    # head=0019：回退 0019(eval 4 表) + 0018(tbox_changes 表) + 0017(加列) + 0016(纯 UPDATE) + 0015(加列) → 0014；
    # 0019/0018 的表被回退 → 表数 = EXPECTED_TABLES - 5；断言退到 0014 后 upgrade 可重放
