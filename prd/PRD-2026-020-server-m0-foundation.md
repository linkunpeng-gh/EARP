...
| **对齐规范** | Tenant Spec v1.2 §5.1（RLS 兜底）+ §5.4（role_id 三层防线，**role_id 的规范权威来源**）；Runtime Spec v1.3 §6.3（Session 基础字段）；Audit Spec v1.1/v1.2 变更（audit_logs.detail 含 role_id/user_roles）；Knowledge Base Spec v1.0 §2.2（Document/Chunk 已锁定列）；data-architecture v1.0（8 域实体/索引/迁移策略）；RBAC 设计 v1.1 §3.1（Role 实体）+ §3.2（RLS SQL 模式）+ §4.3（DDL 清单） |
...
|:---|:---|:------------------------|:---------|
| Workspace | tenants / org_units / users / roles / service_accounts / tenant_account_joins | roles.data_scope VARCHAR（self/department/org/all）+ roles.permissions TEXT[] + roles.knowledge_tags TEXT[]；tenant_account_joins.role_ids TEXT[]、current_role_id VARCHAR | Role 实体：RBAC §3.1；关联表：RBAC §4.3 |
...
| Knowledge | knowledge_bases / documents / chunks | documents.accessible_roles TEXT[]（默认封闭）；chunks 按 KB Spec v1.0 §1.1 已锁定列建表（chunk_id/doc_id/tenant_id/content/embedding/metadata），**M0 仅建列不建向量索引** | KB Spec v1.0；RBAC §3.4 |
