...
--     资料不足时明确说明，不要编造。回答用中文，简洁清晰。」
    ...
    retrieval       JSONB NOT NULL,               -- {mode: vector|hybrid, top_k, threshold}
...
GET    /conversations                    # 会话列表（id/标题/message_count/最后消息时间；对话日志与二期应用形态共用）
GET    /conversations/{id}/messages      # 已存在；响应补 citations 字段
```

### 4.3 chat 链路（chat_service.py，单事务编排）

```
POST /chat_apps/{app_id}/chat  { query, conversation_id? }  → SSE
  ① 校验 app 存在且属于当前租户
  ② 会话：conversation_id 空 → 新建（标题 = 首问截断 30 字）；否则续接
  ③ 落库用户消息（seq++）
  ④ 取最近 context_turns 轮历史（只进 LLM 上下文）
  ⑤ 检索：kb_scope 空 → route_query 软路由；否则限定 KB（角色无权限 KB 静默过滤，不硬拒绝）
  ⑥ 拼提示词 = app.system_prompt + 结构尾巴
  ⑦ LLMConnector.chat_stream 流式生成（DB 模型配置驱动）
  ⑧ 落库助手消息 + citations JSONB
```
...
- **修复 `stream()` 忽略 model_override 的 base_url 问题**（当前直接用 settings.ollama_base_url）；chat_stream 支持 DB model_configs 驱动（provider/model_name/base_url/api_key），无配置回落 env
...
### 5.3 `pages/apps.html` — 应用中心（占位页升级为真实页）
...
**一期**：直接发布（draft → published）+ 审计；应用中心可见。
**二期（本期不做，设计预留）**：
...
1. 发布评审流程 + 可见范围（见 §6）
2. 应用中心最终使用界面（会话列表 + 对话区——本设计早前方案，二期承接）
3. 对话日志引用展示（messages.citations 渲染，P7 二期）
