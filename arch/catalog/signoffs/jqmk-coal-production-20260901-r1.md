# N01A 生产 Catalog Phase 0 签署实例（JQMK 煤矿）
...
| 项目 | 值 |
|---|---|
| 变更单/发布单 | `JQMK-BG-20260901-001` / `JQMK-FB-20260901-001` |
| 目标环境与 tenant/data-domain 范围 | `coal_mining` · tenant=`JQMK`（金桥煤矿）· data-domain=`生产域` |
| `catalog_profile_id` | `coal.sdrh.jqmk.production` |
| `industry_scope` | `coal_mining` |
| enterprise/tenant scope | `SDRH / JQMK` |
| 生效的 platform/industry/enterprise `pack_lock` | HOLD（pack 未形成，version/hash 为 null，见 D-13） |
| 本次签署的 manifest 修订 | HOLD（待生成，见 D-03） |
| `manifest_id` | HOLD（见 D-03） |
| `manifest_schema_version` | HOLD（见 D-03） |
| `manifest_hash` | HOLD（待计算，见 D-03） |
...
| 平台基础 Catalog Pack | 隋昕航 | 平台架构团队 | 隋昕航 | 梁桂岭 | JQMK-BG-20260901-001 |
...
| Metric / Unit / Aggregation 定义 | 隋昕航 | 数据域团队 | 隋昕航 | 梁桂岭 | JQMK-BG-20260901-001 |
| Time Window / Rule Schema 定义 | 隋昕航 | 平台架构团队 | 隋昕航 | 梁桂岭 | JQMK-BG-20260901-001 |
| Binding Template 定义与 resolver | 隋昕航 | 平台架构团队 | 隋昕航 | 梁桂岭 | JQMK-BG-20260901-001 |
...
| Resolver adapter 实现与发布 | 隋昕航 | 平台架构团队 | 隋昕航 | 梁桂岭 | JQMK-BG-20260901-001 |
...
以下事实来自现有 Catalog 边界（**模板固定**）：模型侧引用只能是 exact `CatalogRef={kind,stable_id,version}`；Resolver 必须返回 `content_hash`、`status`、`data_domain_id`、`semantic_schema_version` 及适用的 schema/compatibility 投影。模型 Snapshot/Artifact 保存发布时 pin，不能保存 Provider ID 或目录内部主键。
...
各 kind 的 canonical input、canonicalization/hash 算法及版本未冻结前必须标记 HOLD。
...
|---|---|---|---|---|
| `catalog_profile_id` 与命名规则 | `coal.sdrh.jqmk.production` | 隋昕航 | — | □ APPROVE ☑ HOLD |
...
| platform/industry/enterprise pack identity、version、hash | HOLD（pack 未形成，见 D-13） | 隋昕航 | — | □ APPROVE ☑ HOLD |
...
|---|---|---|
| A. 权威 Resolver 保存 global entry，按调用 tenant/domain 投影授权 | ☑ 采用 | 需证明每次 resolve 都执行 tenant/domain policy 和 RLS 等价隔离（Phase 1 exit 证据）。 |
...
| 解析、选择、审计中的可见性证明 | HOLD（Phase 1 exit 证据） | 责任人：林鲲鹏 |
| 决定 | ☑ APPROVE（机制选 A）；global 启用前子项 HOLD；global 默认拒绝测试移到 Phase 1 exit gate | 林鲲鹏 |
...
以下控制目标在 Phase 0 签署（决定列=控制目标签署）；实现证据在 Phase 1 形成（通过证据列，见 D-14）。当前服务中的 `UnavailableCatalogResolver` 是生产 fail-closed 默认。

| 门禁（控制目标） | 通过证据（Phase 1 exit，D-14） | 责任人 | 决定（控制目标签署） |
|---|---|---|---|
| production/预发禁止 FixtureDiscoveryAdapter/FakeCatalogResolver | HOLD（Phase 1 composition root/部署配置审查） | 梁桂岭 | ☑ APPROVE（签署控制目标） □ HOLD |
| fake 需同时满足 app_env、显式 sandbox/test 标记和 test catalog flag | HOLD（Phase 1 admission policy/启动日志测试） | 梁桂岭+林鲲鹏 | ☑ APPROVE（签署控制目标） □ HOLD |
| production/预发误配 dev/test + flag 时拒绝启动或阻断发布 | HOLD（Phase 1 负向部署测试） | 梁桂岭 | ☑ APPROVE（签署控制目标） □ HOLD |
| test-only Catalog HTTP endpoint 与 fake 使用同一门控 | HOLD（Phase 1 route registry/OpenAPI/访问测试） | 隋昕航 | ☑ APPROVE（签署控制目标） □ HOLD |
| endpoint 不在 production OpenAPI/网络暴露面 | HOLD（Phase 1 生成 spec/网络策略证据） | 隋昕航+梁桂岭 | ☑ APPROVE（签署控制目标） □ HOLD |
| `testing`/`published_fixture` 不能成为 production active candidate | HOLD（Phase 1 activation/Discovery 零写入测试） | 隋昕航 | ☑ APPROVE（签署控制目标） □ HOLD |
| fake fixture hash 变化不能静默重算 | HOLD（Phase 1 fixture manifest 负向测试） | 隋昕航 | ☑ APPROVE（签署控制目标） □ HOLD |
| Case A 45 项回归继续通过 | HOLD（Phase 1 基线 CI 报告） | 隋昕航 | ☑ APPROVE（签署控制目标） □ HOLD |

> 说明：本表"决定"列签署的是**控制目标本身**（Phase 0 承诺这些门禁必须存在）；"通过证据"列 HOLD 表示实现证据在 Phase 1 exit 形成（对应 D-14）。这不代表门禁已实现。

### 8.1 独立审查发现的未闭合门禁

| 未闭合门禁 | 当前边界/不得推断的语义 | 关闭证据 | owner | 决定 |
|---|---|---|---|---|
| production compile outbox consumer 尚无 | 仅有 outbox/event 记录或 compile 调用不等于生产消费、重试和对账已具备 | consumer 部署/运行证据、失败重试与幂等测试、告警/回滚演练（HOLD，Phase 4 exit） | 隋昕航+梁桂岭 | □ APPROVE ☑ HOLD |
| Discovery 必须显式 join N01A active pointers | 不能只依赖 Blueprint `status` 推断可发现性 | join 查询/Resolver contract vector、非 active/漂移/跨域负向测试（HOLD，Phase 4 exit） | 隋昕航 | □ APPROVE ☑ HOLD |
| production separation-of-duties 策略尚未配置 | 不能默认 admin 可自行发布、批准并使 manifest/模型生效 | RBAC policy、相互排斥角色负向测试、审计记录与 break-glass 规则（HOLD，Phase 7 release） | 林鲲鹏 | □ APPROVE ☑ HOLD |
| FixtureDiscoveryAdapter 独立类型/注册尚未发现 | 不得把 fixture adapter 的存在、注册或生产装配当作已确立事实 | 独立 type/registry/composition-root 证据、production 禁止装配测试（HOLD，Phase 1 exit） | 隋昕航 | □ APPROVE ☑ HOLD |
...
| global scope 已选择且有撤销传播 | 机制已选 A；撤销传播 HOLD；global 默认拒绝测试作为 Phase 1 exit 证据（见 §9.2） | 林鲲鹏 | □ APPROVE ☑ HOLD |
...
| fake/test endpoint admission 控制目标已签署 | §8 控制目标已签（本记录）；admission policy 与 deployment negative test 计划证据 HOLD（见 D-14） | 梁桂岭 | ☑ APPROVE □ HOLD |
...
| tenant/domain/global RLS 与权限负向全绿，**含 global 默认拒绝测试** | cross-tenant/cross-domain report + global 默认拒绝负向测试（HOLD，对应 D-04 关闭证据） | 林鲲鹏 | □ APPROVE ☑ HOLD |
...
| D-04 | global scope 实现与撤销传播 | global entry 未启用（机制已选 A）；global 默认拒绝测试移到 Phase 1 exit 关闭 | 林鲲鹏 | Phase 1 exit | §9.2 global 默认拒绝负向测试 | `DEFERRED`（不阻塞当前 profile；关闭证据在 Phase 1 exit） |
...
| D-08 | fake/test endpoint admission 控制目标签署 | Phase 1 entry gate 控制目标 | 梁桂岭 | 2026-09-01（已签） | §8 控制目标已签署（本记录） | `CLOSED`（控制目标签署完成；实现证据见 D-14） |
...
**Phase 0 总结：** `□ READY FOR PHASE 1`　`☑ HOLD`（Owner/RACI 已确认；部分治理方向已签；Phase 1 尚未批准开工。阻塞项：D-01、D-02、D-03、D-13（Phase 1 entry）；D-14、D-12、D-05 告警时限（Phase 1 exit/结束前）。D-04 不阻塞当前 profile（关闭证据在 Phase 1 exit）；D-08 控制目标已签不计入阻塞）
**总负责人：** 林鲲鹏　**日期：** 2026-09-01
**结论/例外说明：**

本实例确认了 Phase 0 的具名 owner（林鲲鹏/隋昕航/梁桂岭）与 RACI（来源 Profile 角色绑定），签署了关键决策：权威存储=git 受控目录 `arch/catalog/`、global scope=机制 A（当前不启用，默认拒绝测试在 Phase 1 exit）、RBAC=ecmc.catalog.read/request/approve、audit=复用 audit_logs、inactive 后已 active 模型的 LKG 运营策略=继续+告警（D-05 决策项已签，告警时限/恢复条件待 Phase 1 前补签）、§8 fake/test 控制目标已签署（D-08 CLOSED，实现证据 D-14 待 Phase 1 exit）。FROZEN 冻结语义均已确认（fail closed、exact ref、非 active 拒绝新操作、approve 不进 fulfilled、callback 幂等/对账等）。manifest 相关（D-01/D-02/D-03）、Catalog Pack（D-13）及后续阶段门禁（Phase 3/4/7）按 HOLD 处理，待对应未决项闭合后补签。D-04（global）DEFERRED，Phase 1 exit 以测试证明 global 默认拒绝。本实例不代表生产 Catalog 已就绪；生产默认保持 fail-closed（UnavailableCatalogResolver），Phase 1 尚未批准开工。
