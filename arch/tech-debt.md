...
| 12 | `ontology/tbox_service.py`（create_entity_type/create_relation_type/deprecate）+ routes | **TBox 变更无强制审批流**：create/deprecate 端点直接生效，设计 §8「TBox 变更必须审批（owner 团队 + 管理员）」仅落在代码注释（audit on caller side）——无实际 gate（无 draft→approved 状态机、无 owner 校验）；seed 类型可被直接停用影响存量实例引用。修复：审批流（draft/active 状态 + 变更审计）或至少 admin 角色门禁 | P2 | TBox 扩展成为高频操作（配合实体导入）或多团队协作时 |
