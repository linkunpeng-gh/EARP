...
### Task 3 — 收藏 API
**文件**：`chat_app_service.py`（或新 `favorites_service.py`）、`main.py`
...
**文件**：`apps/earp-server/src/earp_server/admin/app_categories_routes.py`（新，对齐 roles_routes 模式）、service 或内联
...
**文件**：`admin/app_access_routes.py`（新）、`chat_app_service.py`
...
**文件**：`tests/test_agent_center.py`（新，或拆分进 test_chat_apps.py 等）
...
**文件**：`apps/earp-admin/pages/chat-edit.html`、`chatflow-edit.html`、`js/chat-edit.js`、`js/chatflow-edit.js`
...
### Task 13 — 前端 smoke 测试
**文件**：`apps/earp-admin/test-apps-smoke.cjs`、`test-app-access-smoke.cjs`、`test-app-categories-smoke.cjs`（新，沿用现有 cjs 模式）
...
### Task 14 — 联调 + e2e 验收
**文件**：dev 环境 + 验收记录（session-record 或验收文档）
